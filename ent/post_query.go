...
func (_q *PostQuery) QueryBlocks() *PostBlockQuery {
	query := (&PostBlockClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PostQuery) QueryAnalyticsEvents() *AnalyticsEventQuery {
...
func (_q *PostQuery) WithBlocks(opts ...func(*PostBlockQuery)) *PostQuery {
	query := (&PostBlockClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) WithAnalyticsEvents(opts ...func(*AnalyticsEventQuery)) *PostQuery {
	query := (&AnalyticsEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := _q.withBlocks; query != nil {
		if err := _q.loadBlocks(ctx, query, nodes,
			func(n *Post) { n.Edges.Blocks = []*PostBlock{} },
			func(n *Post, e *PostBlock) { n.Edges.Blocks = append(n.Edges.Blocks, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAnalyticsEvents; query != nil {
		if err := _q.loadAnalyticsEvents(ctx, query, nodes,
			func(n *Post) { n.Edges.AnalyticsEvents = []*AnalyticsEvent{} },
			func(n *Post, e *AnalyticsEvent) { n.Edges.AnalyticsEvents = append(n.Edges.AnalyticsEvents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PostQuery) loadBlocks(ctx context.Context, query *PostBlockQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostBlock)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(postblock.FieldPostID)
	}
	query.Where(predicate.PostBlock(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.BlocksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PostQuery) loadAnalyticsEvents(ctx context.Context, query *AnalyticsEventQuery, nodes []*Post, init func(*Post), assign func(*Post, *AnalyticsEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(analyticsevent.FieldPostID)
	}
	query.Where(predicate.AnalyticsEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.AnalyticsEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PostQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSection != nil {
			_spec.Node.AddColumnOnce(post.FieldSectionID)
		}
