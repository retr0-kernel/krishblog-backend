...
// The PostBlockFunc type is an adapter to allow the use of ordinary
// function as PostBlock mutator.
type PostBlockFunc func(context.Context, *ent.PostBlockMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PostBlockFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PostBlockMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PostBlockMutation", m)
}
